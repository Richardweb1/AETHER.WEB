...
export default function ChatBox({ wallet }: { wallet: string }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => { const updated = [...prev]; const idx = updated.findIndex(m => m.status === "thinking"); if (idx !== -1) updated[idx] = { role: "assistant", content: data.aiResponse, status: "storing" }; return updated; });
      await new Promise(r => setTimeout(r, 1000));
      setMessages(prev => { const updated = [...prev]; const idx = updated.findIndex(m => m.status === "storing"); if (idx !== -1) updated[idx] = { role: "assistant", content: data.aiResponse, status: data.stored_on_shelby ? "stored" : "error", blobName: data.blobName, explorerUrl: data.explorerUrl }; return updated; });
    } catch (error: any) {
