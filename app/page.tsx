...
import { Brain, MessageSquare, Database, Shield, Zap } from "lucide-react";
import ChatBox from "@/components/chat/ChatBox";
import MemoryDashboard from "@/components/dashboard/MemoryDashboard";

export default function Home() {
  const [view, setView] = useState<"chat" | "dashboard">("chat");
  const [wallet, setWallet] = useState<string>("0x789...abcd"); // Simulation
        ...
        <div className="flex items-center gap-4">
          <nav className="flex gap-2">
            <button 
              ...
              className={`px-4 py-2 rounded-lg transition ${view === "chat" ? "bg-violet-600/20 text-violet-400 border border-violet-500/30" : "hover:bg-white/5"}`}
            >
              <MessageSquare className="w-4 h-4 inline mr-2" /> Chat
            </button>
            <button 
              onClick={() => setView("dashboard")}
              className={`px-4 py-2 rounded-lg transition ${view === "dashboard" ? "bg-cyan-600/20 text-cyan-400 border border-cyan-500/30" : "hover:bg-white/5"}`}
            >
